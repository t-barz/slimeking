    ...
    public class SpecialMovementPoint : InteractivePointHandler
    {
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            ...
            if (IsPlayerLayer(other.gameObject.layer))
            {
                ...
                Debug.Log($"SpecialMovementPoint: Player entrou em contato com {movementType} point - {gameObject.name}");
            }
        }
        ...
        private void OnTriggerExit2D(Collider2D other)
        {
            ...
            if (other == _playerCollider)
            {
                ...
                Debug.Log($"SpecialMovementPoint: Player saiu de contato com {movementType} point - {gameObject.name}");
            }
        }
        #endregion
        ...
        private void ValidateConfiguration()
        {
            ...
            {
                Debug.LogWarning($"SpecialMovementPoint: Destination Point não configurado em '{gameObject.name}'. " +
                               "Configure um ponto de destino para o movimento funcionar corretamente.", this);
            }

            // Validação adicional: verificar se destino não é o próprio objeto
            if (destinationPoint == transform)
            {
                Debug.LogError($"SpecialMovementPoint: Destination Point não pode ser o próprio objeto em '{gameObject.name}'. " +
                              "Isso criaria um movimento para o mesmo local.", this);
            ...
            {
                Debug.LogWarning($"SpecialMovementPoint: Movement Duration deve ser maior que 0 em '{gameObject.name}'. " +
                               "Ajustando para 1 segundo.", this);
                movementDuration = 1f;
            }
        }
        ...
        private void InitializeTriggerCollider()
        {
            ...
            {
                Debug.LogWarning($"SpecialMovementPoint: Nenhum Collider2D encontrado em '{gameObject.name}'. " +
                               "Adicionando CircleCollider2D automático.", this);
