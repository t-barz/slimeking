...
namespace SlimeMec.Gameplay
{
    ...
    public class ScreenEffectsManager : MonoBehaviour
    {
        ...
        public Coroutine PlayVignetteTransition(float duration = -1f, float maxIntensity = -1f, System.Action onHalfway = null)
        {
            ...
            {
                Debug.LogWarning("ScreenEffectsManager: Transição de vinheta já em progresso. Interrompendo anterior.");
        ...
        public void ResetVignette()
        {
            ...
            if (_hasVignetteEffect && _vignetteEffect != null)
            {
                ...
                Debug.Log($"ScreenEffectsManager: Vinheta restaurada para intensidade original: {_originalVignetteIntensity:F2}");
            }
        }
        ...
        private IEnumerator VignetteTransitionCoroutine(float duration, float maxIntensity, System.Action onHalfway)
        {
            ...
            Debug.Log($"ScreenEffectsManager: Iniciando transição de vinheta (duração: {duration}s, intensidade: {maxIntensity:F2})");
            ...
            Debug.Log($"ScreenEffectsManager: Transição de vinheta concluída");
        }
        ...
        private IEnumerator VignetteEffectCoroutine(bool closing, float duration, float targetIntensity, System.Action onComplete = null)
        {
            if (!_hasVignetteEffect || _vignetteEffect == null)
            {
                Debug.LogWarning($"ScreenEffectsManager: Efeito de vinheta não disponível, usando delay de {duration:F1}s");
            ...
            string action = closing ? "fechando" : "abrindo";
            Debug.Log($"ScreenEffectsManager: {action} vinheta ({startIntensity:F2} → {targetIntensity:F2}) em {duration:F1}s");
            ...
            Debug.Log($"ScreenEffectsManager: Vinheta {action} - Intensidade final: {_vignetteEffect.intensity.value:F2}");
            onComplete?.Invoke();
        }
        ...
        private void InitializePostProcessing()
        {
            ...
            if (globalVolume == null)
            {
                ...
                {
                    Debug.LogWarning($"ScreenEffectsManager: Global Volume não encontrado. Efeitos de vinheta podem não funcionar.");
                    return;
                }
            }
            else
            ...
            if (_activeGlobalVolume.profile != null)
            {
                if (_activeGlobalVolume.profile.TryGet<Vignette>(out _vignetteEffect))
                {
                    ...
                    Debug.Log($"ScreenEffectsManager: Efeito de vinheta encontrado e configurado. Intensidade original: {_originalVignetteIntensity:F2}");
                }
                else
                {
                    Debug.LogWarning($"ScreenEffectsManager: Efeito de Vignette não encontrado no Global Volume Profile. Adicione um efeito Vignette ao Post-Processing Profile.");
                }
            }
            else
            {
                Debug.LogError($"ScreenEffectsManager: Global Volume não tem Profile configurado. Configure um Post-Processing Profile.");
            }
        }
        ...
        private void ValidateConfiguration()
        {
            ...
            {
                Debug.LogWarning($"ScreenEffectsManager: Default Vignette Transition Time deve ser maior que 0. Ajustando para 1 segundo.");
                defaultVignetteTransitionTime = 1f;
            }

            if (!_hasVignetteEffect)
            {
                Debug.LogWarning($"ScreenEffectsManager: Efeito de vinheta não disponível. Transições visuais podem não funcionar corretamente.");
                hasWarnings = true;
            }

            if (!hasWarnings)
            {
                Debug.Log($"ScreenEffectsManager: Sistema de efeitos de tela inicializado com sucesso!");
            }
        }
        ...
        {
            Debug.Log("ScreenEffectsManager: Reinicializando sistema de pós-processamento...");
            InitializePostProcessing();
        ...
        public void SetPendingTeleport(GameObject playerPrefab, Vector3 position)
        {
            ...
            {
                Debug.LogError("ScreenEffectsManager: Tentativa de configurar teletransporte pendente com prefab nulo!");
            ...
            Debug.Log($"ScreenEffectsManager: Teletransporte pendente configurado - Posição: {position}, Prefab: {playerPrefab.name}");
        }
        ...
        public GameObject ProcessPendingTeleport()
        {
            ...
            Debug.Log($"ScreenEffectsManager: Processando teletransporte pendente...");

            GameObject newPlayer = null;
            if (_pendingPlayerPrefab != null)
            {
                newPlayer = Instantiate(_pendingPlayerPrefab, _pendingPlayerPosition, Quaternion.identity);
                Debug.Log($"ScreenEffectsManager: Player '{newPlayer.name}' instanciado na posição {_pendingPlayerPosition}");

                // Garante que o Player está na cena ativa
                var activeScene = UnityEngine.SceneManagement.SceneManager.GetActiveScene();
                UnityEngine.SceneManagement.SceneManager.MoveGameObjectToScene(newPlayer, activeScene);
                Debug.Log($"ScreenEffectsManager: Player '{newPlayer.name}' movido para a cena ativa '{activeScene.name}'");
            }
            else
            {
                Debug.LogError("ScreenEffectsManager: Player Prefab pendente é null!");
            }
