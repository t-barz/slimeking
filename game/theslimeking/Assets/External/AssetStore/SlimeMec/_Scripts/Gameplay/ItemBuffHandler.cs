...
namespace SlimeMec.Gameplay
{
    ...
    public class ItemBuffHandler : MonoBehaviour
    {
        ...
        public void AddBuff(CollectableItemData itemData)
        {
            ...
            {
                if (enableLogs)
                    Debug.LogWarning($"[ItemBuffHandler] Limite de buffs atingido ({maxBuffs})");
                return;
            }
            ...
            if (enableLogs)
                Debug.Log($"[ItemBuffHandler] Buff adicionado: {newBuff.name} (+{newBuff.attackBonus} ATK, +{newBuff.defenseBonus} DEF, +{newBuff.speedBonus} SPD) por {newBuff.remainingTime}s");
        }
        ...
        public void ClearAllBuffs()
        {
            ...
            if (enableLogs)
                Debug.Log("[ItemBuffHandler] Todos os buffs removidos");
        }
        ...
        private void RemoveBuffAt(int index)
        {
            if (index >= 0 && index < activeBuffs.Count)
            {
                ...
                if (enableLogs)
                    Debug.Log($"[ItemBuffHandler] Buff expirado: {removedBuff.name}");
            }
        }
        ...
        private void RecalculateBuffTotals()
        {
            ...
            if (enableLogs)
                Debug.Log($"[ItemBuffHandler] Buffs recalculados - ATK: +{_totalAttackBuff}, DEF: +{_totalDefenseBuff}, SPD: +{_totalSpeedBuff}");
        }
