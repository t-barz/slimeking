...
public class RandomStyle : MonoBehaviour
{
    ...
    private Color SelectRandomColor()
    {
        if (possibleColors == null || possibleColors.Length == 0)
        {
            if (enableLogs)
            {
                Debug.LogWarning($"RandomStyle em '{gameObject.name}': Lista de cores vazia, usando cor original");
            }
    ...
    private void SetCurrentColor(Color color)
    {
        ...
        {
            Debug.LogWarning($"RandomStyle em '{gameObject.name}': Nenhum componente de renderização encontrado");
        }
    }
    ...
    public void RestoreOriginalStyle()
    {
        ...
        if (enableLogs)
        {
            Debug.Log($"RandomStyle em '{gameObject.name}': Valores originais restaurados");
        }
    }
