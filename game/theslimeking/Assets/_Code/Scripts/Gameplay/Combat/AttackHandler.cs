    ...
    public class AttackHandler : MonoBehaviour
    {
        ...
        public void PerformAttack(bool isAttackingSideways, AttackDirection direction)
        {
            ...
            if (enableDebugLogs)
            {
                ...
                UnityEngine.Debug.Log($"AttackHandler: Ataque {attackType} executado na direção {directionName}, área {effectiveAttackSize}, offset {GetDirectionalOffset()}, {hitCount} objetos detectados");
            }
#endif
        ...
        {UnityEngine.Debug.Log($"Effective Size: {CalculateEffectiveAttackSize(_lastAttackWasSideways)}");// Lista objetos detectados atualmente
            var filter = new ContactFilter2D();
            filter.SetLayerMask(destructableLayerMask);
            filter.useTriggers = true;

            int hitCount = Physics2D.OverlapBox(transform.position, attackSize, 0f, filter, colliderCache);for (int i = 0; i < hitCount; i++)
            {
                Collider2D col = colliderCache[i];
                if (col.gameObject != gameObject)
                {
                    ...
                    UnityEngine.Debug.Log($"  - {col.name} (Tag: {col.tag}, IsDestructable: {hasDestructableTag}, Type: {destructType})");
                }
            }
        }
        #endregion
