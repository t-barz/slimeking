...
Esta documentação descreve a **nova estrutura simplificada** para projetos Unity 2D, projetada para maximizar produtividade com mínima complexidade. A estrutura reduz diversas pastas antigas para **5 pastas de conteúdo + 1 pasta técnica (`Settings/`)** mantendo toda funcionalidade necessária.
...
│   ├── Music/
│   └── SFX/
├── Code/             # Código fonte (Editor / Gameplay / Systems)
├── Game/             # Conteúdo do jogo (Scenes / Prefabs / Data)
├── Settings/         # Configurações do Unity (URP, Project Settings)
└── External/         # Conteúdo de terceiros
```

Nota: `Settings/` é criada pelo Unity e contém assets técnicos (URP, Input, Quality). Não armazenar conteúdo de jogo nela.
...
- 3840 x 2160 (4K) definida via `PlayerSettings` (projeto escalável para monitores menores por downscale)

> Caso deseje desenvolvimento em 1080p, alterar temporariamente em `Project Settings > Player` ou ajustar no script de setup.

### Input System (Opcional)

- O projeto utiliza atualmente API clássica (`Input.GetAxis` etc.) encapsulada em `InputManager`.
- Migração para o novo Input System pode ser feita quando necessário (ver `Setup.md` seção de planejamento de Input).
