...
namespace Water2D
{
    ...
    public class ModernWater2DEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new WaterLayoutUtils.FoldoutScope(FancyEditor, GetAnimBs(12), out var shouldDraw, "performance settings"))
            {
                if (shouldDraw) PerformanceSettings();
            }
        ...
        private void PerformanceSettings() 
        {
            ...
            water.settings._waterSettings.waterType = (WaterType)EditorGUILayout.EnumPopup("water type", water.settings._waterSettings.waterType);
            if(oldS != water.settings._waterSettings.waterType)
            {
                ...
                water.sr.sharedMaterial.SetInt("_cheapWater", cheapEnable?1:0);
                water.sr.sharedMaterial.SetInt("_mobileWater", mobEnable?1:0);
            }
        ...
        private void editorSettings()
        {
            ...
            EndVB();
            */
            //access to managers in editor, use it carefully 
            water.ManagersVisible.value = false;
        }
