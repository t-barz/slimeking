...
public class WindEmulator : MonoBehaviour
{
    ...
    private void InitializeComponents()
    {
        ...
        {
            Debug.LogWarning($"[WindEmulator] Nenhum Animator encontrado em {gameObject.name}!");
            return;
        }
        ...
        if (enableLogs)
        {
            Debug.Log($"[WindEmulator] Inicializado em {gameObject.name} - Trigger: {shakeTriggerName}");
        }
    }
    ...
    private void SetRandomInterval()
    {
        ...
        if (enableLogs)
        {
            Debug.Log($"[WindEmulator] Próximo shake em {currentInterval:F2}s");
        }
    }
    ...
    public void TriggerShake()
    {
        if (cachedAnimator == null)
        {
            Debug.LogWarning($"[WindEmulator] Animator não encontrado em {gameObject.name}!");
            return;
        }
        ...
        if (enableLogs)
        {
            Debug.Log($"[WindEmulator] Shake acionado em {gameObject.name}!");
        }
    }
